...
#we will take both the training and test data subsets and tokenize the titles
...
def transformed_train_data(training_data):
    ...
    bentoml.sklearn.save_model("Tfidf_model",vectorizer,signatures={
        "transform":{
            "batchable":True
        }
    })
...
def xgboost_comments_model(transformed_training_data):
    ...
    with mlflow.start_run():
        ...
        # mlflow.xgboost.log_model(xgb_r, "xgboost_model")
        mlflow.log_param("n_estimators", 20)
    bentoml.sklearn.save_model("XG_model",xgb_r)
    return xgb_r
...
def comments_model_test_set_r_squared(transformed_test_data, xgboost_comments_model):
    ...
    print("transformed_test_data:",transformed_test_data)
   # print("transformed_test_data_shape:",transformed_test_data.shape)
    print("transformed_X_test shape:", transformed_X_test.shape)
    print("transformed_y_test shape:", transformed_y_test.shape)

    with mlflow.start_run(nested=True):  # Nested runs if you want to track separately
        score = xgboost_comments_model.score(transformed_X_test, transformed_y_test)
        mlflow.log_metric("xgboost_comments_model_score", score)
    print('score:',score)
    return score
...
def latest_story_comment_predictions(xgboost_comments_model, tfidf_vectorizer):
    ...
    number_of_comments=xgboost_comments_model.predict(inference_x)
    print("no_of_comments", number_of_comments)
    print("no_of_comments_shape", number_of_comments.shape)
    print("inference_x", inference_x)
    print("inference_x_shape", inference_x.shape)
